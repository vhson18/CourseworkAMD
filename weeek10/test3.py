...
# Function to plot statistics with rotated x-axis labels and adjusted intervals
def plot_statistics(stat, label, title, color='blue'):
    ...
    plt.xticks(ticks=range(0, len(stats.index), 5), labels=list(stats.index[::5]))  # Show every 5th year
    plt.legend()
    plt.grid(True)
    plt.show()
